...
def temporal_only_conv(module, name, removed, stride_removed=0):
    ...
    # Access a parameter of the module to find out its device.
    # Assuming the module has parameters, which Conv3d does (e.g., its weights).
    if hasattr(module, 'weight'):
        device = module.weight.device
        dtype = module.weight.dtype  # Extract the dtype from the weights
    else:
        # Default to CPU and float32 if the module somehow doesn't have parameters
        # This is unlikely for Conv3d but added here for completeness
    ...
    for attr_str in dir(module):
        ...
        if type(sub_module) == nn.Conv3d:
            ...
            param_dict.update({'stride': stride, 'device': device, 'dtype': dtype})
